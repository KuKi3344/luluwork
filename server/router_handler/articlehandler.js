...
exports.publisharticle = (req, res) => {
	...
	db.query(sql, req.body, (err, result) => {
		...
		const sqltxt = "SELECT id FROM articles ORDER BY id DESC";
...
exports.updatearticle = (req, res) => {
	if (req.body.title.length < 1 || req.body.content.length < 1 || req.body.ispub.length < 1 || req.body.categoryid
		.length < 1) return res.send({
		code: 1001,
		message: '参数不合法'
	})
...
exports.getallarticle = (req, res) => {
	const sql =
		'select articles.*,article_cate.name,user.nickname from articles,article_cate,user where articles.state = 1 and articles.ispub = "发布" and user.id = articles.authorid and article_cate.id = articles.categoryid order by articles.firstpub asc'
